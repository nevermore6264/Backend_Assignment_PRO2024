  ...
  revenue_expenditure
    ...
    .catch(err => {
      res.status(500).send({
        message:
          err.message ||
          "Something wrong while creating the Revenue_Expenditure."
      });
    });
};
// Retrieve all staffs from the database.
exports.findAll = (req, res) => {
  Revenue_Expenditure.find()
    ...
    .catch(err => {
      res.status(500).send({
        message:
          err.message ||
          "Something wrong while retrieving Revenue_expenditure. "
      });
    });
};
// Find a single staff with a id
exports.findOne = (req, res) => {
  Revenue_Expenditure.findOne({ _id: req.params.id }).then(
    revenue_expenditure => {
      ...
      return res.status(500).send({
        message:
          " Something wrong retrieving Revenue_Expenditure with id" +
          req.params.id
      });
    }
  );
